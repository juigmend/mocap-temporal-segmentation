...
%                                                                               (JIMG 2018/11/2)
% p.frameformat : format for frames (optional, default = 'png')                 (JIMG 2018/11/2)
%                 for list of formats see 'help imwrite'                        (JIMG 2018/11/2)
...
%     if p.createframes==1                                                                      % commented (JIMG 2018/10/25)
    if p.createframes == 1 && (isfield(p,'frames') == 0 || (isfield(p,'frames') && length(p.frames) > 1)) % (JIMG 2018/10/25)
        mkdir(p.output) %BB20150507 in case frames (series of png files) are needed, and not video file
        cd(p.output) 
%     else %VideoWriter is used with video file output %BB20150507                              % commented (JIMG 2018/10/25)
    elseif p.createframes == 0                                                                            % (JIMG 2018/10/25)
...
% to trace the full sequence:                                   (JIMG 2018/11/4)
if strcmp(p.trl,'full')                                       % (JIMG 2018/11/4)
    p.trl = d.nFrames / d.freq;                               % (JIMG 2018/11/4)
end                                                           % (JIMG 2018/11/4)
...
trlen = round(d2.freq * p.trl);                              % (JIMG 2018/10/29)

% to plot a single frame with trace:                           (JIMG 2018/10/25)
if p.trl && length(n) == 1                                   % (JIMG 2018/10/25)
    n = n - trlen - 1 :  n ;                                 % (JIMG 2018/10/25)
    n = n(n > 0);                                            % (JIMG 2018/10/25)
    mloop_laps = length(n);                                  % (JIMG 2018/10/25)
else                                                         % (JIMG 2018/10/25)
    mloop_laps = 1:length(n);                                % (JIMG 2018/10/25)
end                                                          % (JIMG 2018/10/25)
...
if isfield(p,'frames') == 0                                   % (JIMG 2018/11/4)
    p.frames = [];                                            % (JIMG 2018/11/4)
end                                                           % (JIMG 2018/11/4)
if isfield(p,'frameformat') == 0                              % (JIMG 2018/11/4)
    p.frameformat = [];                                       % (JIMG 2018/11/4)
end                                                           % (JIMG 2018/11/4)
...
% if p.getparams==1                                 % commented (JIMG 2018/11/4)
    ...
    'getparams','perspective','pers','frames','frameformat'});                                % (JIMG 2018/11/4)
...
%     figure(fignr); clf;                          % commented (JIMG 2018/11/2)
    figure(fignr);                                           % (JIMG 2018/11/2)
...
% for k=1:size(x,1) % main loop                    % commented (JIMG 2018/10/25)
for k=mloop_laps % main loop                                   (JIMG 2018/10/25)
    if p.animate
        clf; 
...
%     text(maxxx-650, minzz+350, {YOUR TEXT'}, 'FontSize', 24, 'color', [.15 .15 .15]);
    ...
    % plot traces if animation                     % commented (JIMG 2018/10/25)
%     if p.animate && p.trl~=0                     % commented (JIMG 2018/10/25)

    % plot traces                                  % commented (JIMG 2018/10/25)
    if p.trl                                                 % (JIMG 2018/10/25)
%         trml=sort(p.trm);%BB: sorting marker traces 
%         trlen = round(p.fps * p.trl);            % commented (JIMG 2018/10/25)
        start=max(1,k-trlen);
...
%     text(minxx+70, minzz+0.97*(maxzz-minzz)-75, {'high speed of head'}, 'FontSize', 12, 'FontWeight', 'bold');
    ...
    if p.animate
        if p.createframes==1 
            if isfield(p,'frameformat') == 0                                                     % (JIMG 2018/11/2)
                p.frameformat = 'png';                                                           % (JIMG 2018/11/2)
            end                                                                                  % (JIMG 2018/11/2)
            if strcmp(p.frameformat,'fig')                                                       % (JIMG 2018/11/6) 
                fext = '.fig';                                                                   % (JIMG 2018/11/6)
            elseif strcmp(p.frameformat,'tiff')                                                  % (JIMG 2018/11/2) 
                fext = '.tif';                                                                   % (JIMG 2018/11/2)
            else                                                                                 % (JIMG 2018/11/2)
                fext = ['.',p.frameformat];                                                      % (JIMG 2018/11/2)
            end                                                                                  % (JIMG 2018/11/2)
            if isfield(p,'frames') && length(p.frames) == 1                                      % (JIMG 2018/10/25)
                fn=[p.output,fext];                                                              % (JIMG 2018/10/25)
            else                                                                                 % (JIMG 2018/10/25)
                fn=['frame', sprintf('%0.4d',k),fext]; %old version: create frames
%                 fn=['frame', sprintf('%0.4d',k),'.png']; %old version: create frames % commented (JIMG 2018/11/2)
            end                                                                                  % (JIMG 2018/10/25)               
            if strcmp(p.frameformat,'fig') || strcmp(p.frameformat,'tiff')                       % (JIMG 2018/11/6)     
                figh = get(gca,'Parent');                                                        % (JIMG 2018/11/6)
                if strcmp(p.frameformat,'fig')                                                   % (JIMG 2018/11/6)
                    savefig(figh,fn)                                                             % (JIMG 2018/11/6)
                elseif strcmp(p.frameformat,'tiff')                                              % (JIMG 2018/11/6)
                    figh.PaperPositionMode = 'auto';                                             % (JIMG 2018/11/6)                                             % (JIMG 2018/11/6)
                    print(figh,fn,'-dtiff','-r300')                                              % (JIMG 2018/11/6)
                end
            else                                                                                 % (JIMG 2018/11/2) 
                imwrite(frame2im(getframe),fn,p.frameformat);                                    % (JIMG 2018/11/2)
            end                                                                                  % (JIMG 2018/11/2) 
%             imwrite(frame2im(getframe),fn,'png');                                    % commented (JIMG 2018/11/2)
...
% par = orderfields(par, {'type','scrsize','limits','az','el','msize','colors','markercolors',...                                % commented (JIMG 2018/11/4)
%     'conncolors','tracecolors','numbercolors','cwidth','twidth','conn','conn2','trm','trl',...                                 % commented (JIMG 2018/11/4)
%     'showmnum','numbers','showfnum','animate','fps','output','videoformat','createframes','getparams','perspective','pers'});  % commented (JIMG 2018/11/4)
